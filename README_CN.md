...
也可在`ghost/descript.txt`中写入`githubrepo`字段并后附github项目的url，对于web应用来说两者效果相等且可同时存在  
例子：
...
考虑到uuid可能因为metainfo文件夹地址或homeurl的追加或变更而变化，同一个ghost可能有多个历史uuid。  
...
ghost应当自身携带足够的信息以使得应用可以在不联网的前提下计算出其uuid  
为此，进行以下倡议：  
若ghost有metainfo文件夹，应当在其`descript.txt`中记录其url  
若ghost的metainfo有uuid_base字段，应当在其`descript.txt`中记录  
...
**重要：** 没有metainfo文件夹地址的ghost不应和同名的有metainfo文件夹地址的ghost定义为同一个ghost，直到其通过homeurl或metainfo字段指定了metainfo文件夹地址  
向已存在的ghost追加uuid的方法详见[通过ghost定向到metainfo文件夹](#通过ghost定向到metainfo文件夹)  

### 缺点

uuid只能保护已有homeurl的ghost不被混淆或顶替，但无法保护历史中的无homeurl的ghost不被顶替  
